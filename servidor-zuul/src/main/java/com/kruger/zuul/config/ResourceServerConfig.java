package com.kruger.zuul.config;
...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(HttpMethod.POST, "/api/seguridad/oauth/**").permitAll()
				.antMatchers(HttpMethod.GET, "/api/empleado/**").hasAnyAuthority("EMPLEADO")
				.antMatchers(HttpMethod.POST, "/api/administracion/**").hasAnyAuthority("ADMINISTRADOR")
				.antMatchers(HttpMethod.GET, "/api/administracion/**").hasAnyAuthority("ADMINISTRADOR").anyRequest()
				.authenticated();
	}
